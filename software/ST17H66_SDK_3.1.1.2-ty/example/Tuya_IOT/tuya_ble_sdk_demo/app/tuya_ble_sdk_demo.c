...
#endif
            //接收DP点数据
            static uint16_t send_sn = 0;

            uint8_t dp_send_buf[5] = {0};

            dp_send_buf[0] = event->dp_received_data.p_data[0];
            dp_send_buf[1] = 4; //枚举类型
            dp_send_buf[2] = 0;
            dp_send_buf[3] = 1;
            dp_send_buf[4] = 0; //枚举内容只有一个，所以固定为0
            
            //上报数据，让日志中有记录
            tuya_ble_dp_data_send(send_sn++, DP_SEND_TYPE_ACTIVE, DP_SEND_FOR_CLOUD_PANEL, DP_SEND_WITHOUT_RESPONSE, dp_send_buf, 5);

            extern void app_ctr_rc_send_once(uint8_t index);

            if(event->dp_received_data.p_data[0] == 101)
            {
                app_ctr_rc_send_once(1);
            }
            else if(event->dp_received_data.p_data[0] == 102)
            {
                app_ctr_rc_send_once(3);
            }
            else if(event->dp_received_data.p_data[0] == 103)
            {
...
static tuya_ble_timer_t chx_key_pro_timer;
#define CHX_KEY_PRO_TIMER_TIME_MS 10

//static void ty_ble_sdk_key_event_cb(key_event_t *e)
//{
//    if(e->event == KEY_EVENT_INT)
//    {
//        tuya_ble_chx_key_pro_timer_start();
//    }
//    else if(e->event == KEY_EVENT_LONG)
//    {
//        //绑定状态就接触绑定
