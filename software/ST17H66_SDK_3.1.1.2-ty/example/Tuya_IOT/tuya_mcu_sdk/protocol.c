...
{
  {DPID_OPEN, DP_TYPE_ENUM},
  {DPID_CLOSE, DP_TYPE_ENUM},
  {DPID_PAUSE, DP_TYPE_ENUM},
};
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_open_handle
功能描述 : 针对DPID_OPEN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_open_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char open;
    
    open = mcu_get_dp_download_enum(value,length);
    switch(open) {
        case 0:

        extern void ty_rec_open_cmd(void);
        ty_rec_open_cmd();
    ...
    ret = mcu_dp_enum_update(DPID_OPEN, open);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_close_handle
功能描述 : 针对DPID_CLOSE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_close_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char close;
    
    close = mcu_get_dp_download_enum(value,length);
    switch(close) {
        case 0:

        extern void ty_rec_close_cmd(void);
        ty_rec_close_cmd();
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_CLOSE, close);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
static unsigned char dp_download_pause_handle(const unsigned char value[], unsigned short length)
{
    ...
    pause = mcu_get_dp_download_enum(value,length);
    switch(pause) {
        case 0:

        extern void ty_rec_pause_cmd(void);
        ty_rec_pause_cmd();
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_PAUSE, pause);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_OPEN:
            //打开处理函数
            ret = dp_download_open_handle(value,length);
        break;
        case DPID_CLOSE:
            //关闭处理函数
            ret = dp_download_close_handle(value,length);
        break;
        case DPID_PAUSE:
            //停止处理函数
